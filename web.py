...
def index():
    ...
    link += "<a href=/movie2>讀取開眼電影即將上映影片，寫入Firestore</a><br>"
    link += "<a href=/searchQ>片名關鍵字查詢</a><hr>"
...
def webhook():
    ...
    #info = "我是黃千育設計的電影聊天機器人, 動作：" + action + "； 查詢內容：" + msg
    if (action == "rateChoice"):
        rate =  req["queryResult"]["parameters"]["rate"]
        info = "我是黃千育設計的電影聊天機器人,您選擇的電影分級是：" + rate + "，相關電影：\n"
        db = firestore.client()
...
def search():
    ...
    docs = collection_ref.get()
    Temp = ""
    keyword = request.args.get("teacher_name", "")
    if keyword:
        for doc in docs:
            user = doc.to_dict()
            # 4. 檢查 "name" 欄位是否存在，並比對關鍵字
            if keyword in user["name"]:
                Temp += f"{user['name']}老師的研究室是在{user.get('lab', '未提供')}<br>"
        if not Temp:
            Temp = "查無此老師資訊。"
